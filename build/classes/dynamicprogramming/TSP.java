...
public class TSP implements ITSP {
    ...
    public void solve() {
        ...
        System.out.println(minCost(vertex, 1));
    ...
    private double minCost(int[] subset, int j) {
        ...
        for (int k = 0; k < subset.length; k++) {
            ...
            double value = cost(subsetMinus(subset, j), i, j) + adjacencyMatrix[i][j];
            if (result > value) {
                result = value;
            }
        }
    ...
    private double cost(int[] subset, int i, int j) {
        if (subset.length == 1) {
            if (subset[0] == 1 && i == 1) {
                return 0;
            }
