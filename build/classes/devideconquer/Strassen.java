...
public class Strassen implements IStrassen {
    ...
    @Override
    public double[][] strassenMultiply(double[][] first, double[][] second) {
        int n = first.length;        
        if(!isPoweofTwo(n)){
    ...
    @Override
    public double[][] devide(double[][] matrix, int row, int col) {
        int n = matrix.length;
    ...
    @Override
    public void conquer(double[][] matrix, double[][] result, int row, int col) {
        int n = result.length;
        int numOfRows = (row == 1) ? n / 2 : (n - n / 2);
        int numOfCols = (col == 1) ? n / 2 : (n - n / 2);
        int rowIndex = (row == 1) ? 0 : n / 2;
